...
<section>
<form method="post" action="gestion_location.php" >
<fieldset>
<legend> AFFICHAGE DES RECLAMATIONS </legend>
...
{		
     while ($cpt <= $id_max )
        { 
          if ( isset($_POST[$cpt]) and !empty($_POST['etat_demande'])) 
           { $_POST['etat_demande']=htmlspecialchars($_POST['etat_demande']);
		     $rep = $bdd->prepare('UPDATE location SET etat_demande=:etat_demande  WHERE id =:id_a_modif');
...
//Definie la partie du forum demande 
    ...
    while($i<$nbr_liens)
	{ 
	   echo('<th scope="col"> <a href=gestion_location.php?index_page='.$i.'&amp;type_engin='.$type_engin.'&amp;marque_engin='.$marque_engin.'&amp;etat_demande='.$etat_demande.'>'.($i+1).'</a></th>');
	   $i++;
	}
	?>
	</tr>
 ...
 <option value="annuler"> Annuler </option>
	...
	<input type="submit" name="afficher_commandes" value="Confirmer" /> 
	<a href="http://127.0.0.1/MTP_pro/Admin/gestion_location.php"> <input type="button" name="actualiser" value="Actualiser"/></a>
	 <br/>
     </fieldset>
	</form>	
